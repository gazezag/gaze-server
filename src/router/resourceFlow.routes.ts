...
import { resourceFlowController } from '../controller/resourceFLow.controller';

const router = new Router({ prefix: '/resource-flow' });
...
router
  ...
  .get('/empty.gif', async (ctx, next) => {
    const data: ResourceFlowDTO = JSON.parse(decodeURIComponent(ctx.search).slice(1));

    // data is stored and pushed here
    await resourceFlowController(data);
    await next();
  })
  /**
